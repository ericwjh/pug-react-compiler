...
Rectifier.prototype = {
  ...
  visitProgram: function(program) {
    return this.scope(function() {
      ...
      if (!body.some(b.isCallExpression)) {
        body = [b.callExpression(this.domCall('span'), [b.objectExpression()].concat(body))];
      }
      ...
      if (this.scope.requires.length) {
        // var seen = {};
        this.scope.requires.forEach(function(dtor) {
          dtors.push(dtor)
        })
      }
      program.body = dtors.concat(program.body)
  ...
  visitFunction: function(fun) {
    return this.scope(function() {
      ...
      // fun.body = b.blockStatement(body)
  ...
  visitWhileStatement: function(whiles) {
    whiles.test = this.visit(whiles.test);
    whiles.body = this.visit(whiles.body);
    return whiles;
  },
  ...
  visitArrowFunctionExpression: function(fun) {
    return this.visitFunction(fun);
    ...
    if (b.isIdentifier(call.callee)) {
      switch (call.callee.name) {
        case 'ǃDOM＿':
          ...
          // callee
          call.callee = this.domCall(this.visit(property));
          // attributes
          ...
          args = args.map(function(it) {
            ...
            if (b.isIfStatement(it) || b.isConditionalExpression(it) ||
              b.isCallExpression(it) && (b.isIdentifier(it.callee) &&
                (it.callee.name[0] === 'ǃ' &&
                  it.callee.name[it.callee.name.length - 1] === '＿' || it.callee.name === '__map'
                )
              ) ||
      ...
      if (name.indexOf('data-') === 0) {
        if (b.isLiteral(value)) {
          ...
          it.value = b.callExpression(
            b.memberExpression(
              b.identifier('JSON'),
              b.identifier('stringify'), false), [value]);
        }
      }
      props.push(it);
      return props;
  ...
  commonJS: function(body) {
    var exprs = this.returnable(body)
    if (this.options.filename.indexOf('.jade')>-1) {
      return [
      ...
      b.expressionStatement(
        b.assignmentExpression('=',
          b.memberExpression(
            ...
            "id": {
              "type": "Identifier",
              "name": (this.options.filename || '__Component').replace(/[^\w]/g, '_')
            },
            "superClass": {
